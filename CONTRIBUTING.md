...
versioned `YYYY.M.N` and tagged `v2026.9.0` (no leading zero: PEP 440 strips it
and asset URLs are formatted from the version). Everything in the package is
derived from the checkout by `build_registry.py`, which CI runs on every PR.
Every model directory must appear in `catalog.py`'s `MODEL_MAP`; a directory
with several `scene*.xml` and no `scene.xml` names its default in
`PREVIEW_OVERRIDES`.
